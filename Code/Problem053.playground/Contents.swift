...
class Solution {
    ...
    func maxSubArray(_ nums: [Int]) -> Int {
        let temp = getSum(nums)
        if temp.allBig == true {
            return temp.sum
        }
        if temp.allSmall == true {
            return temp.max
        }
        var result = temp.sum
    ...
    func getSum(_ nums: [Int]) -> (sum: Int, max: Int, allBig: Bool, allSmall: Bool) {
        var sum = 0
        var max = Int.min
        var allBig = true
        var allSmall = true
